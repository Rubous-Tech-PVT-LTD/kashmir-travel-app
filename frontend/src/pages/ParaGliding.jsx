  ...
  return (
    <div className="bg-[#f8fbff]">
      ...
      <section className="relative overflow-hidden text-white py-20 px-6 bg-linear-to-br from-[#0a1929] via-[#1e5a8e] to-[#3b8bc9]">
        ...
        <div className="absolute w-70 h-70 -right-25 -top-20 rounded-full bg-[radial-gradient(circle,rgba(100,200,255,0.44),transparent)] animate-pulse"></div>
        <div className="absolute w-75 h-75 -left-22.5 -bottom-22.5 rounded-full bg-[radial-gradient(circle,rgba(200,220,255,0.34),transparent)]"></div>

        <div className="max-w-285.5 mx-auto relative">
          <button
            onClick={() => navigate('/')}
          ...
          <div className="grid md:grid-cols-2 gap-8 items-center">
            <div>
              <p className="text-xs tracking-[1.8px] text-[#a8d5ff] mb-3">
                KASHMIR PARAGLIDING
              </p>
              ...
              <p className="text-[#d8edf8] leading-relaxed mb-6">
                Experience Kashmir's most stunning aerial view with safe tandem paragliding and cinematic adventure moments.
              </p>
              ...
              <div className="grid grid-cols-3 gap-3">
                {flightMoments.map((item) => (
                  <div key={item.label} className="bg-white/10 rounded-lg p-3">
                    <p className="text-[11px] text-[#a8d5ff] font-bold">{item.label}</p>
      ...
      {/* HIGHLIGHTS */}
      <section className="max-w-287.5 mx-auto px-6 py-14 grid md:grid-cols-2 gap-6">
        <div>
      ...
      <section className="bg-[#eef7fc] py-14 px-6">
        ...
        <div className="max-w-287.5 mx-auto grid md:grid-cols-3 gap-5">
          {flightPackages.map((item) => (
            <div key={item.name} className="bg-white p-5 rounded-xl shadow hover:-translate-y-1 transition">
              <p className="text-xs text-[#1e5a8e] font-bold">{item.duration}</p>
              <h3 className="text-lg font-semibold">{item.name}</h3>
              <p className="text-sm text-gray-600 mb-4">{item.note}</p>

              <div className="flex justify-between items-center">
                <span className="font-bold">{item.price}</span>
                <button className="bg-[#1e5a8e] text-white px-4 py-2 rounded">
                  Reserve
                </button>
              </div>
            </div>
      ...
      <section className="max-w-287.5 mx-auto px-6 py-14 grid md:grid-cols-2 gap-6">
        ...
        <div className="bg-linear-to-br from-[#10263b] via-[#1e5a8e] to-[#3b8bc9] text-white rounded-2xl p-6">
          <img
            src="https://picsum.photos/id/10/1200/800"
            className="rounded-xl mb-4 h-62.5 w-full object-cover"
          />
          ...
          <p className="text-sm mb-4">
            Combine paragliding with your Kashmir trip plan.
          </p>
